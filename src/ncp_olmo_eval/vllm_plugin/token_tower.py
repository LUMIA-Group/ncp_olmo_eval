...
def _rope_parameters(config: Any, *, apply_yarn: bool) -> dict[str, Any]:
    ...
    desired_mscale = _yarn_mscale(factor, mscale) / _yarn_mscale(
...
def _sliding_window(config: Any, layer_number: int) -> int | None:
    ...
    if not isinstance(skip_frequency, int) or isinstance(skip_frequency, bool):
        raise ValueError("the first OLMo3 backend requires integer window_attn_skip_freq")
...
class ConceptLMOlmo3Attention(nn.Module):
    """OLMo3 MHA using vLLM Attention and native Megatron parameter names."""
...
class ConceptLMOlmo3MLP(nn.Module):
    """OLMo3 SwiGLU MLP with native Megatron fused FC1 naming."""
    ...
    def _record_stage(self, name: str, value: torch.Tensor) -> None:
        if self._trace_layer_index is not None:
            self._latest_stage_trace[
                f"hlm.mlp.{name}.{self._trace_layer_index}"
...
class ConceptLMTokenBackbone(nn.Module):
    """Token-only ConceptLM backbone with exact native checkpoint parameter paths."""
        ...
        *,
        ...
        if vllm_config.quant_config is not None:
            raise NotImplementedError(
                "native Megatron ConceptLM weights are initially supported unquantized"
            )
    ...
    def embed_input_ids(self, input_ids: torch.Tensor) -> torch.Tensor:
        """Apply native ConceptLM token embeddings."""
    ...
    def load_weights(
        ...
        """Load token parameters from native or dedicated-HF checkpoint keys."""
