"""Fail-closed configuration contract for the native ConceptLM backend."""
...
ARCHITECTURE = "ConceptLMV22VQForCausalLM"
MODEL_TYPE = "conceptlm_v22_vq"
STANDALONE_HF_ARCHITECTURE = "NCPOlmo3ForCausalLM"
STANDALONE_HF_MODEL_TYPE = "ncp_olmo3"
WEIGHT_KEY_FORMAT = "native_megatron_state_dict"
HF_WEIGHT_KEY_FORMAT = "huggingface_state_dict"
SUPPORTED_WEIGHT_KEY_FORMATS = frozenset(
    (
        WEIGHT_KEY_FORMAT,
        HF_WEIGHT_KEY_FORMAT,
        "hf_state_dict",
        "native_hf_state_dict",
        "ncp_olmo3_hf_state_dict",
    )
)
SUPPORTED_VLLM_VERSION = "0.13.0"
...
def normalize_qk_norm_config(
    ...
    *,
    ...
    if raw_mode is None:
        ...
        raise BackendContractError(
            "qk_norm_mode must be one of full_hidden, per_head, got "
            f"{raw_mode!r}"
        )
    ...
    if raw_weight_size is None:
        ...
        raise BackendContractError(
            "qk_norm_weight_size must be a positive integer, got "
            f"{raw_weight_size!r}"
        )
    else:
...
class ConceptLMBackendConfig:
    ...
    def from_mapping(cls, config: Mapping[str, Any]) -> "ConceptLMBackendConfig":
        ...
        supported_identity = (
            ...
            and (
                (model_type == MODEL_TYPE and ARCHITECTURE in architectures)
                or (
                    model_type == STANDALONE_HF_MODEL_TYPE
                    and STANDALONE_HF_ARCHITECTURE in architectures
                )
            )
        )
        if not supported_identity:
            raise BackendContractError(
                ...
                "expected "
                f"({ARCHITECTURE!r}, {MODEL_TYPE!r}) or "
                f"({STANDALONE_HF_ARCHITECTURE!r}, {STANDALONE_HF_MODEL_TYPE!r})"
            )
        weight_key_format = _required(config, "weight_key_format")
        if weight_key_format not in SUPPORTED_WEIGHT_KEY_FORMATS:
            raise BackendContractError(
                "weight_key_format must be one of "
                f"{sorted(SUPPORTED_WEIGHT_KEY_FORMATS)!r}, got "
                f"{weight_key_format!r}"
            )
        if _required(config, "conceptlm_backbone") != "olmo3":
            raise BackendContractError(
                "the first native backend only supports conceptlm_backbone='olmo3'"
            )
        ...
        if hlm_attention_mode == "backbone_window":
            ...
            if (
                ...
                or any(isinstance(item, bool) or not isinstance(item, int) for item in window_size)
            ):
                raise BackendContractError(
