...
class ConceptLMDDTwoRouteAdd(nn.Module):
    """Decoder self-DD and final-concept modules with native names."""
...
class ConceptLMStage3Routes(nn.Module):
    ...
    def decoder_after_layer(
        ...
        *,
        ...
        if self.cumsum_routes:
            ...
            hidden_states, cumsum_state = (
                self.dd_two_route_add.decoder_cumsum_dd(
                    raw_output,
                    cumsum_state,
                )
            )
    ...
    def load_weights(
        ...
        for checkpoint_name, loaded_weight in weights:
            ...
            if target is None:
                continue
            if target.shard_id is not None or target.deinterleave_megatron_qkv:
                raise ValueError(
