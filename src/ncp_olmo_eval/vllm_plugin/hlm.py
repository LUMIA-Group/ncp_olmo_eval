...
from .qkv import deinterleave_megatron_qkv
from .state import ConceptRequestState, HLMKVState, append_tensor_buffer
from .token_tower import ConceptLMOlmo3MLP, _config_value, _rope_parameters, _sliding_window
...
def _append_hlm_kv(
    ...
    if kv_state.key is None:
        ...
        if required_length > capacity:
            ...
            new_key[:, :old_length].copy_(
                kv_state.key[:, :old_length]
            )
            new_value[:, :old_length].copy_(
                kv_state.value[:, :old_length]
            )
...
class ConceptLMDepthDD(nn.Module):
    ...
    def forward(
        ...
        if (
            history_states.ndim < 2
            or history_states.shape[-2] != self.static_a.numel()
        ):
            raise ValueError(
...
class ConceptLMSelfDD(nn.Module):
    """Per-layer DD modules with native Megatron checkpoint names."""
...
class ConceptLMHLMIncrementalAttention(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        use_yarn = (
            str(_config_value(raw_config, "position_embedding_type")) == "yarn"
            and (
        ...
        self.rotary_emb = get_rope(
            ...
            is_neox_style=not bool(
                _config_value(raw_config, "rotary_interleaved")
            ),
...
class ConceptLMHLMIncrementalLayer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self._trace_enabled = bool(
            os.environ.get("CONCEPTLM_VLLM_LOGITS_TRACE_DIR")
        )
    ...
    def _record_stage(self, name: str, value: torch.Tensor) -> None:
        if self._trace_enabled:
            self._latest_stage_trace[
                f"hlm.{name}.{self.layer_index}"
...
class ConceptLMHLMBlock(nn.Module):
    """Locally numbered HLM block with native checkpoint paths."""
...
class ConceptLMHighLevelBranch(nn.Module):
    ...
    def advance(
        ...
        for layer_index, layer in enumerate(
            self.concept_predictor.hlm_block.layers
        ):
        ...
        hidden_states = self.concept_predictor.hlm_block.final_layernorm(
            hidden_states
        )
    ...
    def advance_batch(
        ...
        if any(
            int(source.shape[0]) != batch_size
            for source in encoder_layer_chunks
        ):
            raise ValueError("all HLM encoder sources must match the batch size")
        concept_positions = {
            request_state.predicted_concepts.length
            for request_state in request_states
        }
        if len(concept_positions) != 1:
            raise ValueError(
                "batched HLM requests must have the same concept position"
            )
    ...
    def load_weights(
        ...
        """Load HLM parameters from native or dedicated-HF checkpoint keys."""
        ...
        for checkpoint_name, loaded_weight in weights:
            ...
            if (target.shard_id is None and target_shards) or (
                ...
                raise ValueError(
                    "cannot mix full and split HLM tensors for "
            ...
            if target.deinterleave_megatron_qkv:
                query, key, value = deinterleave_megatron_qkv(
                    loaded_weight,
                    num_attention_heads=self.backend_config.num_attention_heads,
                    num_key_value_heads=self.backend_config.num_key_value_heads,
                )
                weight_loader(parameter, query, "q")
                weight_loader(parameter, key, "k")
                weight_loader(parameter, value, "v")
            elif target.shard_id is not None:
