...
from ncp_olmo_eval.vllm_plugin.contract import BackendContractError, ConceptLMBackendConfig
...
class TestConceptLMBackendContract(unittest.TestCase):
    ...
    def test_huggingface_weight_key_format_is_supported(self) -> None:
        hf_config = copy.deepcopy(valid_config())
        hf_config["weight_key_format"] = "huggingface_state_dict"

        normalized = ConceptLMBackendConfig.from_mapping(hf_config)

        self.assertEqual(normalized.encoder_layers, 16)

    def test_standalone_hf_model_identity_is_supported(self) -> None:
        hf_config = copy.deepcopy(valid_config())
        hf_config["architectures"] = ["NCPOlmo3ForCausalLM"]
        hf_config["model_type"] = "ncp_olmo3"
        hf_config["qk_norm_mode"] = "per_head"
        hf_config["qk_norm_weight_size"] = 128
    ...
    def test_mixed_model_identity_fails(self) -> None:
        broken = copy.deepcopy(valid_config())
        broken["architectures"] = ["NCPOlmo3ForCausalLM"]
        with self.assertRaisesRegex(BackendContractError, "model identity"):
            ConceptLMBackendConfig.from_mapping(broken)

    def test_unknown_weight_key_format_fails(self) -> None:
        broken = copy.deepcopy(valid_config())
        broken["weight_key_format"] = "unknown"
        with self.assertRaisesRegex(BackendContractError, "weight_key_format"):
            ConceptLMBackendConfig.from_mapping(broken)
